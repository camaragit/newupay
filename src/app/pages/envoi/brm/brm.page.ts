...
export class BrmPage implements OnInit {
  ...
  ionViewDidEnter(){
    this.serv.get(this.glb.URLBANQUE+'brm/getlist').then((data)=>{
      ...
      this.databrm.controls.pays.setValue(this.codepays[0]);
      this.onselectpays();
      this.databrm.controls.prenomexpediteur.setValue(this.glb.PRENOM);
      this.databrm.controls.nomexpediteur.setValue(this.glb.NOM);
      } 
    })
  }
  onselectpays(){
    const country = this.databrm.controls.pays.value;
